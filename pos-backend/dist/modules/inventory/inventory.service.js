...
class InventoryService {
    ...
    async restockProduct(data) {
        try {
            ...
            console.log(`📦 Restocked ${product.name} (SKU: ${product.sku}): ${previousStock} → ${updatedProduct.stock} (+${data.quantity})`);
            return {
                success: true,
                product: {
    ...
    async adjustStock(data) {
        try {
            ...
            console.log(`🔧 Adjusted stock for ${product.name} (SKU: ${product.sku}): ${previousStock} → ${updatedProduct.stock}`);
