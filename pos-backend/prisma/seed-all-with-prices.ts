...
// Helper to calculate price based on category and type if needed, 
// but we will hardcode reasonable market prices (USD estimates).
...
const allProducts = [
    ...
    { name: 'Scuds', sku: 'CIG-SCUDS', category: 'Cigarettes', price: 0.20, description: 'Loose Cigarettes (Scuds)' }, // Assuming single or cheat pack
...
async function main() {
    console.log(`🚀 Updating prices for ${allProducts.length} products...\n`)
    ...
    for (const product of allProducts) {
        const created = await prisma.product.upsert({
            ...
            update: {
                ...
                stock: 20, // Ensuring stock is set
        ...
        console.log(`  ✅ ${created.name.padEnd(25)} | $${created.price.toString().padEnd(6)} | ${created.description}`)
    }

    console.log(`\n🎉 All prices and details updated successfully!`)
}
